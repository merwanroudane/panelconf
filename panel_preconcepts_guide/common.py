"""
Shared palette, styling and UI helpers for the
"Panel Data Pre-Concepts" researcher guide (light theme only).
...
PALETTE = {
    ...
    "red": "#E8590C", "pink": "#E64980", "grape": "#9C36B5", "violet": "#7048E8",
    "gray": "#495057", "ink": "#1F2733", "muted": "#5C6B7A",
    "paper": "#FFFFFF", "soft": "#F4F6FC",
}
...
def inject_css():
    st.markdown(
        ...
        .hero { border-radius:20px; padding:34px 40px; color:#fff; margin-bottom:22px;
                box-shadow:0 14px 40px -18px rgba(80,40,180,.5); }
        .hero h1 { font-size:2.1rem; margin:0 0 .35em; line-height:1.12; font-weight:800; }
        .hero p  { font-size:1.05rem; margin:0; opacity:.96; line-height:1.55; }
        .hero .tag { font-size:.72rem; letter-spacing:.18em; text-transform:uppercase;
                     font-weight:700; opacity:.85; margin-bottom:.9em; }

        .sec { border-left:6px solid var(--acc); padding:.1em 0 .1em 16px; margin:1.6em 0 .4em; }
        .sec h2 { margin:0; font-size:1.45rem; font-weight:800; color:var(--ink); }
        ...
        .card h4 { margin:0 0 .35em; font-size:1.02rem; font-weight:700; color:var(--ink); }
        .card p  { margin:.2em 0; color:var(--muted); font-size:.94rem; line-height:1.55; }
...
def hero(title, subtitle, color, tag="Panel Data Pre-Concepts"):
...
def chips(items, color):
    html = "".join(
        ...
        f'--chipfg33:{color}44">{it}</span>' for it in items)
...
def dev_footer():
    st.markdown(
        """<div class="devfoot">
        Developed by <b>Dr Merwan Roudane</b> &nbsp;·&nbsp; author of the <b>xt*</b> panel
        structural-break Stata suite &nbsp;·&nbsp; ideas.repec.org/f/pro1421
        &nbsp;·&nbsp; github.com/merwanroudane</div>""",
        unsafe_allow_html=True)
...
def sidebar_brand():
    with st.sidebar:
        st.markdown(
            ...
            <div style="font-weight:800;font-size:1.02rem;color:#1F2733">📘 Panel Pre-Concepts</div>
            <div style="font-size:.78rem;color:#8A97A6">Foundations before 3rd generation</div>
            <div style="font-size:.72rem;color:#8A97A6;margin-top:6px">
            by <b>Dr Merwan Roudane</b></div></div>""",
            unsafe_allow_html=True)
